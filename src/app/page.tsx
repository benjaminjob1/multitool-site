...
export default function Multitool() {
  ...
  return (
    <div className="min-h-screen text-white flex flex-col" style={{ backgroundColor: "rgb(0,0,0)", filter: `brightness(${settings.brightness})` }}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-20 h-14 flex items-center px-2" style={{ backgroundColor: "rgba(0,0,0,0.95)" }}>
...
function RulerTool() {
  const [cm, setCm] = useState(15); const px = 40;
  return (
    <div className="p-6 flex flex-col items-center justify-center min-h-[400px] gap-6">
      <div className="flex items-center gap-3">
        <button onClick={() => setCm(Math.max(5,cm-5))} className="w-12 h-12 rounded-xl border border-white/10 hover:bg-white/5 font-bold">-5</button>
        <button onClick={() => setCm(Math.max(1,cm-1))} className="w-12 h-12 rounded-xl border border-white/10 hover:bg-white/5 font-bold">-</button>
        <div className="text-2xl font-bold w-20 text-center">{cm} cm</div>
        <button onClick={() => setCm(Math.min(50,cm+1))} className="w-12 h-12 rounded-xl border border-white/10 hover:bg-white/5 font-bold">+</button>
        <button onClick={() => setCm(Math.min(50,cm+5))} className="w-12 h-12 rounded-xl border border-white/10 hover:bg-white/5 font-bold">+5</button>
      </div>
...
function ARRulerTool({s}: {s: ToolSettings}) {
  ...
  useEffect(() => {
    if(pts.length===2 && vRef.current?.videoWidth) { const dx=pts[1].x-pts[0].x; const dy=pts[1].y-pts[0].y; const px2=Math.sqrt(dx*dx+dy*dy); if(px2>0) setDist((ref*800)/px2); }
